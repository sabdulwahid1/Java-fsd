...
<c:if test="${not empty result.rows}"> 
	...
	<tr> 
		<td>Status:</td> 
		<td><input type="radio" name="status" value="open" ${row.status == 'open' ? 'checked' : ''} required> Open 
		<input type="radio" name="status" value="closed" ${row.status == 'closed' ? 'checked' : ''} required> Closed</td> 
	</tr>
